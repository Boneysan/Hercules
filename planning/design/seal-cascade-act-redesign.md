...
8. **Recovery is part of the feature.** Existing quest/flag helpers synchronize online members; the present durable checkpoint still does not replay missed quest/flag events. Story completion is per character, so each enrolled party character needs an ordered event cursor and safe reconnect replay; alternate characters on the same account receive no inherited chapters. Follow the [DM party quest-sync contract](../../../korangar/docs/specs/dm-party-quest-sync.md). DM-only repair must preview changes, never replay grants, and never silently overwrite an unrelated campaign.
